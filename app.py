...
if st.session_state.stage == 'intro':
    ...
    st.image("https://r.jina.ai/i/06be6331908a4628a50f111534f40f0c", use_container_width=True)
    with st.container():
        st.subheader("가디언 등록")
...
# [엔딩 화면]
    ...
    st.balloons()
    st.title("🏆 미션 클리어!")
    st.subheader(f"{st.session_state.user_info.get('dept', '')} {st.session_state.user_info.get('name', '')} 가디언님")
    st.write(f"최종 점수: {st.session_state.score} / 300")
    st.info("귀하의 기록은 부서 KPI 점수에 안전하게 반영되었습니다.")
    
    if st.checkbox("관리자용 데이터 확인"):
        try:
            df = pd.read_csv("compliance_results.csv")
            st.table(df)
        except:
