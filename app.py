...
def render_mcq_question(m_key: str, q_idx: int, q_data: dict):
    ...
    if st.button("제출하기", key=f"submit_mcq_{m_key}_{q_idx}", use_container_width=True, type="primary"):
...
def render_text_question(m_key: str, q_idx: int, q_data: dict):
    ...
    if st.button("제출하기", key=f"submit_text_{m_key}_{q_idx}", use_container_width=True, type="primary"):
...
def render_quiz_navigation_controls(m_key: str):
    ...
    with c2:
        if idx < total_q - 1:
            if st.button("다음 문제 ▶", key=f"nav_next_{m_key}_{idx}", use_container_width=True, type="primary", disabled=(not current_submitted)):
...
with st.sidebar:
    ...
    for _ in range(6):
        st.write("")
    if st.button("🔐 관리자 대시보드", use_container_width=True, type="primary"):
...
try:
    if st.session_state.stage == "intro":
        ...
        c1, c2 = st.columns([1, 1], gap='large')
        with c1:
            if st.button("✅ 최종 제출(Submit)", use_container_width=True, type="primary"):
